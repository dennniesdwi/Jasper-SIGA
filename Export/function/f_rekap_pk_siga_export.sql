...
begin
	return query
		SELECT COALESCE(COALESCE(rek.kki, pk.kki), bkb.kki)::text AS kki,
            ...
            COALESCE(rek.nama_provinsi, pk.nama_provinsi) as nama_provinsi,
            COALESCE(rek.nama_kabupaten, pk.nama_kabupaten) as nama_kabupaten,
            COALESCE(rek.nama_kecamatan, pk.nama_kecamatan) as nama_kecamatan,
            COALESCE(rek.nama_kelurahan, pk.nama_kelurahan) as nama_kelurahan,
            COALESCE(rek.nama_rw, pk.nama_rw) as nama_rw,
            COALESCE(rek.nama_rt, pk.nama_rt) as nama_rt,
           ...
           FROM 
			...
			left join (select * from (
					...
					WHERE concat(a.tahun_rekap::varchar, lpad(a.bulan_rekap::varchar,2,'0')) <= coalesce(concat(v_tahun::varchar, lpad(v_bulan::varchar,2,'0')), to_char(now(), 'YYYYMM'))
					) b
				where b.flag is null) dk
			on hd.id_rekap = dk.id_rekap
			left join sigabaru."Provinsi" w1
			on hd.id_provinsi = w1.id_provinsi
			left join sigabaru."Kabupaten" w2
			on hd.id_kabupaten = w2.id_kabupaten
			left join sigabaru."Kecamatan" w3
			on hd.id_kecamatan = w3.id_kecamatan
			left join sigabaru."Kelurahan" w4
			on hd.id_kelurahan = w4.id_kelurahan
			left join sigabaru."RW" w5
			on hd.id_rw = w5.id_rw
			left join sigabaru."RT" w6
			on hd.id_rt = w6.id_rt
			where (v_nik is null
...
--				and upper(di.hubungan_dengan_kk) = 'KK'
			...
			left join sigabaru."Provinsi" w1
			on a.id_provinsi = w1.id_provinsi
			left join sigabaru."Kabupaten" w2
			on a.id_kabupaten = w2.id_kabupaten
			left join sigabaru."Kecamatan" w3
			on a.id_kecamatan = w3.id_kecamatan
			left join sigabaru."Kelurahan" w4
			on a.id_kelurahan = w4.id_kelurahan
			left join sigabaru."RW" w5
			on a.id_rw = w5.id_rw
			left join sigabaru."RT" w6
			on a.id_rt = w6.id_rt
...
--				and upper(a.hubungan_dengan_kk) = 'KK'
             ...
             FULL JOIN ( SELECT *
                   FROM ( SELECT a.*,
                          ...
                          WHERE concat(tahun_rekap::varchar, lpad(bulan_rekap::varchar,2,'0')) <= coalesce(concat(v_tahun::varchar, lpad(v_bulan::varchar,2,'0')), to_char(now(), 'YYYYMM'))
                          ) b
	...
	order by kki, (case when COALESCE(COALESCE(rek.hubungan_dengan_kk, pk.hubungan_dengan_kk::character varying), 'KK') = 'KK' then 1
		...
		else 4 end), COALESCE(rek.tanggal_lahir, pk.tanggal_lahir)
	;
end;
$function$
;
...
ALTER FUNCTION sigabaru.f_rekap_pk_siga_export(int4,int4,varchar,varchar,int4,int4,int4,int4,int4,int4,bool,bool,bool,varchar,varchar,bool,bool,bool,bool,bool,bool,bool,bool,bool,bool) OWNER TO developer;
GRANT ALL ON FUNCTION sigabaru.f_rekap_pk_siga_export(int4,int4,varchar,varchar,int4,int4,int4,int4,int4,int4,bool,bool,bool,varchar,varchar,bool,bool,bool,bool,bool,bool,bool,bool,bool,bool) TO developer;
